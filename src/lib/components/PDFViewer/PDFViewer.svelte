<script lang="ts">
	...
	export let data: Uint8Array | null = null;
	export let scale: number = 1.8;
	export let pageNum: number = 1;
	export let flipTime: number = 120;
	export let showButtons: string[] = [
	  'navigation',
	  'zoom',
	  'print',
	  'rotate',
	...
	export let showTopButton: boolean = true;
	export let onProgress: ((progress: { loaded: number; total: number }) => void) | undefined = undefined;
	export let externalLinksTarget: string = '_blank';
	...
	const handlePageLinks = async (page: PDFPage, viewport: Viewport): Promise<void> => {
	  try {
		console.log('handlePageLinks', page, viewport);
	...
	const createLinkOverlay = (annotation: PDFAnnotation, viewport: Viewport): void => {
	  console.log('createLinkOverlay', annotation, viewport);
	...
	const clockwiseRotate = (): void => {
	  console.log('clockwiseRotate', rotation);
	  rotation += 90;
	  queueRenderPage(pageNum);
	};

	const antiClockwiseRotate = (): void => {
	  console.log('antiClockwiseRotate', rotation);
	  rotation -= 90;
	  queueRenderPage(pageNum);
	};
	  ...
	  {:else if showButtons.length}
		<div class="control-start">
		  ...
		  <div class={showBorder === true ? 'viewer' : 'null'}>
			...
			<canvas bind:this={canvas} width={750} height={750}></canvas>
		  </div>
		</div>
	  {:else}
		<div class={showBorder === true ? 'viewer' : 'null'}>
