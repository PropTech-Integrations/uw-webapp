<script lang="ts">
  ...
  <div class="metric-widget h-full flex flex-col justify-center">
    <p class="text-sm text-gray-600 mb-1">{widgetData.label}</p>
    <p class="text-3xl font-bold text-gray-900">
      {widgetData.value}{widgetData.unit ? ` ${widgetData.unit}` : ''}
    </p>
    {#if widgetData.change !== undefined}
      <p class="text-sm mt-2 flex items-center {widgetData.changeType === 'increase' ? 'text-green-600' : 'text-red-600'}">
