<script lang="ts">
    ...
    const getMenuItems = (widget: Widget): Array<{action: WidgetAction, label: string, icon: string, divider?: boolean}> => {
      const items = [];
    ...
    let dropdownStyle = $derived(buttonRect ? `
      position: fixed;
  ...
  <div class="widget-dropdown relative">
    <!-- Dropdown Button -->
    <button
      bind:this={buttonEl}
      onclick={toggleDropdown}
      class="dropdown-button absolute top-2 right-2 z-20 p-1.5 bg-white/80 hover:bg-white rounded-md shadow-sm border border-gray-200 opacity-0 group-hover:opacity-100 transition-opacity"
    ...
    {#if isOpen && buttonRect}
      <div
        ...
        class="dropdown-menu w-48 bg-white rounded-lg shadow-xl border border-gray-200 py-1 animate-in"
        style={dropdownStyle}
      >
        {#each menuItems as item}
          ...
          {:else}
            <button
              ...
              class="menu-item w-full px-3 py-2 text-left text-sm text-gray-700 hover:bg-gray-50 hover:text-gray-900 flex items-center gap-2 {item.action === 'remove' ? 'text-red-600 hover:bg-red-50 hover:text-red-700' : ''}"
            >
