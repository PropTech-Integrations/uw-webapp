...
import { mapStore, createProducer, createConsumer, createProducerConsumer } from '../src/lib/stores/mapObjectStore';
...
class UserService {
  ...
  updatePreferences(preferences: Partial<User['preferences']>) {
    this.user.update(current => {
      if (!current) throw new Error('Cannot update preferences: no user data');
      return {
        ...current,
...
class ProductService {
  ...
  private productsConsumer = mapStore.registerConsumer<Product[]>('products', 'productService-consumer');

  getProducts() {
    return this.productsConsumer.get();
  }
...
class CartService {
  ...
  addToCart(productId: string) {
    ...
    this.cart.update(cart => {
      if (!cart) throw new Error('Cannot add to cart: cart not initialized');
  ...
  removeFromCart(productId: string) {
    this.cart.update(cart => {
      if (!cart) throw new Error('Cannot remove from cart: cart not initialized');
...
class SystemBridges {
  ...
  constructor(notificationService: NotificationService, productService: ProductService) {
...
async function runApplication() {
  ...
  // Set up bridges
  const bridges = new SystemBridges(notificationService, productService);
